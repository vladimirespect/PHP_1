...
include "config/classSimpleImage.php";



define('DIR_ROOT', $_SERVER['DOCUMENT_ROOT']);
define('BIG', DIR_ROOT . '/img/gallery_img/big/');
define('SMALL', DIR_ROOT . '/img/gallery_img/small/');

function upload() {
    include "widgets/upload.php";
}
    ...
    <div class="gallery">
        <? while ($row = mysqli_fetch_assoc($result)) : ?>
            <a class="link" href="/bigImage.php?id=<?= $row['id'] ?>"><img
                        src="/img/gallery_img/small/<?= $row['name'] ?>" width="150"/></a>
        <? endwhile; ?>
    </div>
    <?= $message ?>
