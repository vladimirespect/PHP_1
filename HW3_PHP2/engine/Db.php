...
//это самый нижний уровень sql запросов
...
class Db
{
    ...
    public function queryOne($sql, $params = []) //в модели и сыновьях есть метод getOne который вызывает метод queryOne
    {
        return $this->query($sql, $params)->fetch();//fetch() вернёт в том виде который задали setAttribute(\PDO::ATTR_DEFAULT_FETCH_MODE, \PDO::FETCH_ASSOC) , т.е. в виде ассоциативного массива данные
    }
...
//$rows_affected = $STH->rowCount();  возвращает кол-во затронутых строк
//$DBH->lastInsertId(); Метод ->lastInsertId() возвращает id последней вставленной записи